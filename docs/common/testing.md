...
Тест создается функцией `testWidgets`, которая в качестве параметра получает инструмент,
с помощью которого код теста взамодействует с тестируемым виджетом.
```dart
...
Тесты можно объеденять в группы, внутри функции `group`:
```dart
...
В отличии от виджет тестов, процесс работы интеграционного теста можно наблюдать в симуляторе или на экране устройства.
...
В этом примере показан так же код создания и освобождения инструмента, через который тесты взаимодействуют
с тестируемым приложением


### Взаимодействие с тестируемым приложением

Инструмент `FlutterDriver` взаимодействовать с тестируемым приложением через следующие методы:
...
Поиск виджетов при интеграционном тестировании не значительно отличается от аналогичной функциональности при тестировании виджетов
...
Этот метод тестирование может хорошо заменить ручное тестирование.
...
Имеется проблема, когда тесты завершаются и в симуляторе остается запущенное приложение,
